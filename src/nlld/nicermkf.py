...
def define_nicerdet_cartxy():
    """
    Create cartesian [x, y] coordinates of detectors based on the index
    :return: cart_coord_nicerdet_df
    :rtype cart_coord_nicerdet_df: pandas.DataFrame
    """
    nicDET_geograph = define_nicerdetloc()
    cart_coord_nicerdet = np.zeros([7, 8], dtype=object)

    for ii in range(7):
        for jj in range(8):
            cart_coord_nicerdet[ii, jj] = np.array([jj, ii])

    cart_coord_nicerdet_df = pd.DataFrame(data=np.flipud(cart_coord_nicerdet).flatten(), index=nicDET_geograph)
    cart_coord_nicerdet_df.rename(columns={0: 'Det_xy'}, inplace=True)
    cart_coord_nicerdet_df.index = cart_coord_nicerdet_df.index.astype(int)
